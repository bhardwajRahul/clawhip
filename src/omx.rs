...
pub async fn run(args: OmxLaunchArgs, config: &AppConfig) -> Result<()> {
    ...
    eprintln!(
        "clawhip omx session={session_name} workdir={} project={project}",
        workdir.display()
    );

    let omx_flags = env::var("CLAWHIP_OMX_FLAGS")
        .unwrap_or_else(|_| args.omx_flags.clone().unwrap_or_else(|| "--madmax".into()));

    let omx_command = build_omx_shell_command(&session_name, &project, &workdir, &omx_flags, &args);

    // Build keywords
    let keywords: Vec<String> = if args.keywords.is_empty() {
        env::var("CLAWHIP_OMX_KEYWORDS")
            .map(|v| v.split(',').map(String::from).collect())
            .unwrap_or_else(|_| DEFAULT_OMX_KEYWORDS.iter().map(|s| s.to_string()).collect())
    } else {
        args.keywords.clone()
    };

    let stale_minutes = args.stale_minutes.unwrap_or_else(|| {
        env::var("CLAWHIP_OMX_STALE_MIN")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(DEFAULT_OMX_STALE_MINUTES)
    });
    ...
    if let Some(prompt) = args.prompt.clone() {
        ...
        tokio::spawn(async move {
            ...
            if let Err(e) = deliver_prompt(&target_session, &prompt).await {
                eprintln!("clawhip omx: prompt delivery failed: {e}");
            }
        });
    }
...
/// lifecycle event emission via native clawhip hook-envelopes and the actual
/// `omx` invocation.
fn build_omx_shell_command(
    session: &str,
    project: &str,
    workdir: &Path,
    omx_flags: &str,
    ...
    let omx_env = env::var("CLAWHIP_OMX_ENV").unwrap_or_default();
    let env_prefix = if omx_env.is_empty() {
        String::new()
    } else {
        format!("{omx_env} ")
    };
...
cleanup() {{
  ...
  if [ "$exit_code" -eq 0 ]; then
    clawhip emit agent.finished --agent omx --session {session_esc} --project {project_esc} --elapsed "$elapsed"{emit_suffix} || true
  else
    clawhip emit agent.failed --agent omx --session {session_esc} --project {project_esc} --elapsed "$elapsed" --error "exit $exit_code"{emit_suffix} || true
  fi
}}
trap cleanup EXIT
...
mod tests {
    ...
    fn sanitize_tmux_session_name_replaces_special_chars() {
        ...
        assert_eq!(sanitize_tmux_session_name("feat+omx"), "feat-omx");
